...
function main {
    ...
    cd object_detection/
    pip install yacs cython matplotlib opencv-python pycocotools
    ./install.sh
    cd ../
    mkdir -p datasets
    ...
    do
        # cache
        python object_detection/pytorch/./tools/test_net.py --device $device \
            --config-file object_detection/pytorch/configs/e2e_mask_rcnn_R_50_FPN_1x.yaml \
            --batch_size 1 \
...
function generate_core_launcher {
    ...
    do
        ...
        printf "python -m oob-common.launch --enable_jemalloc \
            ...
            object_detection/pytorch/./tools/test_net.py --device $device \
                --config-file object_detection/pytorch/configs/e2e_mask_rcnn_R_50_FPN_1x.yaml \
                --batch_size $batch_size \
                --num_iter $num_iter --num_warmup $num_warmup \
                --precision $precision \
