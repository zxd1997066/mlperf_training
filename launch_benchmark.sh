...
function main {
    ...
    do
        # pre run
        python train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --threshold=0.23 --data ${DATASET_DIR} \
            --evaluation 1  --perf-prerun-warmup 1 --arch ssd300 -b 1 --perf-run-iters 2 \
    ...
    do
        ...
        printf " ${OOB_EXEC_HEADER} \
            python train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --arch ssd300 \
                --threshold=0.23 --data ${DATASET_DIR} --evaluation 1 \
...
function generate_core_launcher {
    ...
    do
        ...
        printf "python -m oob-common.launch --enable_jemalloc \
            ...
            train.py --epochs 1 --warmup-factor 0 --lr 2.5e-3 --no-save --arch ssd300 \
                --threshold=0.23 --data ${DATASET_DIR} --evaluation 1 \
                --perf-prerun-warmup ${num_warmup} --perf-run-iters ${num_iter} \
                -b ${batch_size} \
                --precision ${precision} \
